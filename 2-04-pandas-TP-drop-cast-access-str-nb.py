...
# %%
df = lune.drop(columns = 'Unnamed: 0')
...
# %%
#La colonne des masses contient des éléments de type 'object' et pas 'float' ou 'int'.
...
# %%
#Elle ne contient que des chaînes de caractères avec des entiers ou des '>entier' ou des '<entier'
...
# %% [raw]
# df['Mass (lb)'] = pd.to_numeric(df['Mass (lb)'], errors = 'coerce', downcast = 'integer')
# df['Mass (lb)']
...
# %%
# votre code
mask = df['Mass (lb)'].isna() 
df['Mass (kg)'] = df['Mass (lb)']/2.205
df['Mass (kg)'].astype(dtype = 'int', copy = True, errors = 'ignore')
#ça ne marche pas mais je ne saiis pas pourquoi
...
for country in C :
    ...
    print(country, mask.sum()*100/len(mask), '%')
...
# %%
df['Mass (kg)'][df['Country'] == 'United States'].sum()
...
# %%
# votre code
'Memorial' in str(df['Artificial object'])
