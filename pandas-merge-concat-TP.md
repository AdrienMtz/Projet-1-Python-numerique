...
```{code-cell} ipython3
df4 = df1.merge(df2, left_index = True, right_on = 'name')
df4
```
...
m4 = m1.merge(m2, left_index = True , right_on = ['first_name', 'last_name'])
m4
```
...
pd.concat([m4.set_index(['first_name', 'last_name']),m3.set_index(['first', 'last'])])
```
