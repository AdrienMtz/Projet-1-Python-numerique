...
# %%
df = df.sort_index(axis = 0, level = 'Date').set_index('Date')
df
...
# %%
rolling_points = s.rolling(window = 10, center = True).mean()
pd.DataFrame({
    'original': s,
    'roll 28D': rolling_28,
    'roll points' : rolling_points,
}).plot();

# %% [markdown]
# Aux extrémités, il n'y a pas de valeurs puisque la moyenne est glissante, donc elle ne peut être calculée que s'il y a 10 points répartis équitablement autour du point considéré. Cette condition n'est pas vérifiée aux extrémités.
...
# %%
# à vous
df2.resample(rule = 'W').mean().plot()

# %%
df2.resample('W').mean().count()

# %% [markdown]
# Il n'y a plus que 79 valeurs, ce qui représente plus du 7e du nombre de valeurs dans la dataframe d'origine, car il y avait moins d'une valeur par jour. 
...
# %%
# à vous
df[['High']].rolling(window = pd.Timedelta(365,'D'), center = True).mean().plot()
...
# 8 heures avant ça, i.e. 4 heures avant d1
t2 - 2 * d1
